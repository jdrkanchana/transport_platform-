...
def central():
    ...
    result = mycursor.execute("SELECT status FROM coordinator_dead where zone_status=%s",(status,))
    result = mycursor.fetchall()
    for row in result :
        status=row[0]
        print("jelly")
        if status=="dead":
            print("Electing new central master")
    issueTic=issueTicket(vmdID)
    print(issueTic)
    print("angels n demons")
    dk=issueTic
